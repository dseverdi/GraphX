...
class graph:
  ...
  def edges(self):
    ...
    for secondary_map in self._outgoing.values():
      result.update(secondary_map.values())    # dodaj bridove u skup
    return result
  ...
  def insert_vertex(self,v,x=None):
    """Ubaci novi brid. """
    V = self._getVertex(v)
    if V in self._outgoing.values() : raise ValueError('Vrh je vec u grafu')
  ...
  def show(self):
    ...
    for u in self._outgoing.keys():
        ...
        for v in self._outgoing[u].keys():
            if self._outgoing[u][v].element(): print "-[%2.0d]-" % (self._outgoing[u][v].element()),
            print "(%s) " % str(v.key()),
